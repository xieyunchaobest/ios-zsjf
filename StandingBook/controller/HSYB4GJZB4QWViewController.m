...
@interface HSYB4GJZB4QWViewController ()

@end

@implementation HSYB4GJZB4QWViewController
...
- (void)viewDidLoad
{
    ...
    [self setcurrentSwitchStyle:self.switchczsrButton];
    ...
    [super setWrapTitle:@"全网关键指标月报" date:date];
...
-(void)initSwitchButton{
    ...
    [self setSwitchButtonStyle:self.self.switchczsrButton];
...
-(void) doAfterinitData:(NSData*)responseData {
    ...
    for (int i = 0; i < [array  count]; i ++) {//行数
        ...
        if([reportFlag isEqualToString:@"czsr"]){
            for (NSString *key in trDict) {
                ...
                if([key isEqualToString:@"0"]){
                    ...
                    rfa=[vo objectForKey:@"qwCzsrDy"];
                }else if([key isEqualToString:@"2"]){
                    rfa=[vo objectForKey:@"qwCzsrSy"];
                }else if([key isEqualToString:@"3"]){
                    rfa=[vo objectForKey:@"qwCzsrHbzzl"];
                }else if([key isEqualToString:@"4"]){
                    rfa=[vo objectForKey:@"qwCzsrTbzzl"];
                }else if([key isEqualToString:@"5"]){
                    rfa=[vo objectForKey:@"qwCzsrDbzzl"];
                }
            ...
            for (NSString *key in trDict) {
                ...
                if([key isEqualToString:@"0"]){
                    ...
                    rfa=[vo objectForKey:@"qwCzyhsDbzzl"];
                }
                [data setValue:rfa forKey:key];
            }
        }
...
- (IBAction)chooseDate:(UIDatePicker*)sender {
    ...
    if (self.myDatePicker.datePickerMode == UIDatePickerModeTime) {
        ...
        NSString *selectDate =  [dateFormatter stringFromDate:select];
        [super setWrapTitle:@"全网关键指标月报" date:selectDate];
        NSLog(@"selectDateselectDate%@",selectDate);
