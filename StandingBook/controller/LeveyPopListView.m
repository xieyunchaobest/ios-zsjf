...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions {
    ...
    if (self = [super initWithFrame:rect]) {
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(POPLISTVIEW_SCREENINSET, 
                                                                   ...
                                                                   rect.size.height - 2 * POPLISTVIEW_SCREENINSET - POPLISTVIEW_HEADER_HEIGHT - RADIUS-20)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_options[indexPath.row] respondsToSelector:@selector(objectForKey:)]) {
        ...
        cell.textLabel.text = _options[indexPath.row][@"diqu"];
...
- (void)drawRect:(CGRect)rect {
    ...
    [[ UIColor colorWithRed:211.0/255 green:12.0/255 blue:1.0/255 alpha:1.0] setFill];
    ...
    //CGContextSetShadowWithColor(ctx, CGSizeMake(0, 1), 0.5f, [UIColor  redColor].CGColor);
