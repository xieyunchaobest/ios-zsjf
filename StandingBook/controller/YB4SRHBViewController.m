...
@interface YB4SRHBViewController ()

@end

@implementation YB4SRHBViewController
...
- (void)viewDidLoad
{
    ...
    [super setWrapTitle:@"收入环比通报(按客户群)" date:date];
...
-(void)initData:(NSMutableDictionary*)requestDic{
    ...
    NSString* subUrl=@"";
    subUrl=[[[NSString alloc] initWithString:@"tm/ZSJFAction.do?method=showYB4Srhbtbkhq"] autorelease];
//    NSMutableDictionary* requestData = [[NSMutableDictionary alloc] init];
//    //[requestDic setObject:adate forKey:@"date"];
...
-(NSMutableArray*)transDataType:(NSArray*) arr{
    ...
    for(int i=0;i<arr.count;i++){
        ...
        NSString *dq=[dd objectForKey:@"khq"];
        if(![@"合计" isEqualToString:dq ]){
            NSMutableDictionary *md=[[NSMutableDictionary alloc] initWithDictionary:dd];
            
            NSString *g2s=[dd objectForKey:@"zzDysr"];
            int a=[g2s intValue];
            [md setValue:[NSNumber numberWithInt:a] forKey:@"zzDysr"];
            
            NSString *g3s=[dd objectForKey:@"clDysr"];
            int b=[g3s intValue];
            [md setValue:[NSNumber numberWithInt:b] forKey:@"clDysr"];
            
            NSString *kdfzrb=[dd objectForKey:@"jlSysr"];
            int c=[kdfzrb intValue];
            [md setValue:[NSNumber numberWithInt:c] forKey:@"jlSysr"];
            
            NSString *zdywKdcjdylj=[dd objectForKey:@"zlDysr"];
            int d=[zdywKdcjdylj intValue];
            [md setValue:[NSNumber numberWithInt:d] forKey:@"zlDysr"];
            
            NSString *a2g3gfz=[dd objectForKey:@"czSr"];
            int e=[a2g3gfz intValue];
            [md setValue:[NSNumber numberWithInt:e] forKey:@"czSr"];
...
- (IBAction)chooseDate:(UIDatePicker*)sender {
    ...
    if (self.myDatePicker.datePickerMode == UIDatePickerModeTime) {
        ...
        [super setWrapTitle:@"收入环比通报(按客户群)" date:[selectDate substringWithRange:NSMakeRange(0,7)]];
        NSLog(@"selectDateselectDate%@",selectDate);
