...
@interface QDRB4QdwgzdywrbViewController ()

@end

@implementation QDRB4QdwgzdywrbViewController
...
- (void)viewDidLoad
{
    ...
    [super setWrapTitle:@"渠道网格重点业务日报" date:date];
...
-(void)initData:(NSMutableDictionary*)requestDic{
    ...
    NSString* subUrl=[[[NSString alloc] initWithString:@"tm/ZSJFAction.do?method=qdrb4qdwgzdywrb"] autorelease];
...
- (IBAction)chooseDate:(id)sender {
    ...
    if (self.myDatePicker.datePickerMode == UIDatePickerModeTime) {
        ...
        NSString *selectDate =  [dateFormatter stringFromDate:select];
        [super setWrapTitle:@"渠道网格重点业务日报" date:selectDate];
        NSLog(@"selectDateselectDate%@",selectDate);
