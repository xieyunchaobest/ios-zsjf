...
@interface YB4SRHB4ywViewController ()

@end

@implementation YB4SRHB4ywViewController
...
- (void)viewDidLoad
{
    ...
    [super setWrapTitle:@"收入环比通报(按业务)" date:date];
...
-(void)initData:(NSString*)adate{
    ...
    NSString* subUrl=@"";
    subUrl=[[[NSString alloc] initWithString:@"tm/ZSJFAction.do?method=showYB4srhbtb4yw"] autorelease];
...
- (IBAction)chooseDate:(id)sender {
    ...
    if (self.myDatePicker.datePickerMode == UIDatePickerModeTime) {
        ...
        NSString *selectDate =  [dateFormatter stringFromDate:select];
        [super setWrapTitle:@"收入环比通报(按业务)" date:[selectDate substringWithRange:NSMakeRange(0,7)]];
